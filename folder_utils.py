...
# @Last Modified time: 2021-03-03 14:25:00
...
def fullpath_to_last3folders_path(files):
    '''
    Remove the fullpath of a list of files
    Input: 
        files: list of strings
    ...
    return [f.split(os.sep)[-3] + os.sep + f.split(os.sep)[-2] + os.sep + f.split(os.sep)[-1] for f in files] #  os.path.split returns (head, tail). Only tail is desired
...
def filter_folders_prefix(valid_prefixs, folder_list):
    '''
    Filter a list of files with a list of prefix
    Input: 
        valid_prefixs: list of strings
        folder_list: list of strings
    Output: 
        listv: list of strings
    '''
    listv = []
    for prefix in valid_prefixs:
